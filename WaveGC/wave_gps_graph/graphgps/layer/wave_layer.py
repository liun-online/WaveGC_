...
class GraphWavelet(nn.Module):
    def __init__(self, dim_h, act, dropout, n_scales):
        super().__init__()
        self.dim_h = dim_h
        self.single_out = self.dim_h//(n_scales+1)
        self.single_out_last = self.dim_h - n_scales*self.single_out
        self.activation = register.act_dict[act]()
        self.drop = nn.Dropout(dropout)
        self.t_number = n_scales + 1
        ...
        '''
        self.wavelet_conv_1 = nn.ModuleList([pygnn.GCNConv(dim_h, dim_h, normalize=False)  for _ in range(self.t_number)])
        self.fusion = nn.Linear(self.t_number*dim_h, dim_h)
        '''
    ...
    def forward(self, x, batch):
        ...
        for t in range(self.t_number):
            attr = batch.bases[t].to(torch.float32)
...
class GraphWavelet_V2(nn.Module):
    def __init__(self, dim_h, act, dropout, n_scales, adj_dropout,layer_idx):
        super().__init__()
        self.dim_h = dim_h
        self.single_out = self.dim_h//(n_scales+1)
        self.single_out_last = self.dim_h - n_scales*self.single_out
    ...
    def forward(self, x, batch):
        ...
        index = batch.full_index.to(torch.int64)
        curr_signals = batch.filter_signals_after[:, self.layer_idx] # B*N*(n_scales+1)
        diag_filter_signals_after = torch.diag_embed(curr_signals.transpose(2, 1))  # B*(n_scales+1)*N*N
        for t in range(self.t_number):
...
class WaveLayer(nn.Module):
    ...
    def __init__(self, layer_num, layer_idx,dim_h,
                 local_gnn_type, global_model_type, num_heads, n_scales, act='relu',
                 pna_degrees=None, equivstable_pe=False, dropout=0.0, adj_dropout=0.1,
                 attn_dropout=0.0, layer_norm=False, batch_norm=True,
                 bigbird_cfg=None, log_attn_weights=False, wave_drop=0.):
        super().__init__()
        ...
        if global_model_type == 'None':
        ...
        elif global_model_type == 'genwave':
            if cfg.lam.ham:
                if layer_idx!=0 and layer_idx!=layer_num-1:
                    self.self_attn = GraphWavelet(dim_h, act, wave_drop, n_scales)
            ...
            if cfg.lam.ham:
                if layer_idx!=0 and layer_idx!=layer_num-1:
                    self.self_attn = GraphWavelet_V2(dim_h, act, wave_drop, n_scales, adj_dropout,layer_idx-1)
                    self.attn_flag = 'wavelet'
                else:
                    self.self_attn = torch.nn.MultiheadAttention(
                        dim_h, num_heads, dropout=self.attn_dropout, batch_first=True)
                ...
                self.self_attn = GraphWavelet_V2(dim_h, act, wave_drop, n_scales, adj_dropout,layer_idx)
                self.attn_flag = 'wavelet'
        else:
