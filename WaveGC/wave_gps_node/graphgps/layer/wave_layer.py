...
class GraphWavelet(nn.Module):
    def __init__(self, dim_h, act, dropout, n_scales, adj_dropout):
        super().__init__()
        self.dim_h = dim_h
    ...
    def forward(self, x, batch):
        ...
        for t in range(self.t_number):
            ...
            sele_filters = torch.abs(filters) > cfg.lam.keep_thre
...
class GraphWavelet_V2(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        evc_dense = batch.eigenvector.unsqueeze(0)
        evc_dense_t = evc_dense.transpose(1, 2)
        curr_signals = batch.filter_signals_after[:, self.layer_idx] # 1*sele_num*(n_scales+1)
        diag_filter_signals_after = torch.diag_embed(curr_signals.transpose(2, 1))  # 1*(n_scales+1)*sele_num*sele_num
        for t in range(self.t_number):
            filters = evc_dense @ diag_filter_signals_after[:, t, :, :] @ evc_dense_t
            filters = F.normalize(self.adj_dropout(filters))
        ...
        if global_model_type == 'None':
            ...
            if cfg.lam.ham:
                if layer_idx!=0 and layer_idx!=layer_num-1:
                    self.self_attn = GraphWavelet(dim_h, act, wave_drop, n_scales, adj_dropout)
